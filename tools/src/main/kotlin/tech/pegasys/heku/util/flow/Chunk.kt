...
fun <T> Flow<T>.chunkedWithPeriod(scope: CoroutineScope, period: Duration): Flow<List<T>> {
    return flow {
        var accum = mutableListOf<T>()
        val timerCh = ticker(period.inWholeMilliseconds, period.inWholeMilliseconds)
        val dataCh = this@chunkedWithPeriod.toChannel(scope)
        while (true) {
            select<Unit> {
                timerCh.onReceive {
                    emit(accum)
