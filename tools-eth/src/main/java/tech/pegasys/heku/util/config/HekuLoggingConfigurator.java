...
import tech.pegasys.teku.infrastructure.logging.ConsolePatternSelector;
...
public class HekuLoggingConfigurator {
  ...
  public void startLogging(final LoggingConfigExt configuration) {
    update(configuration);
  }

  public static synchronized void update(final LoggingConfigExt configEx) {
    LoggingConfig configuration = configEx.getLogConfigBuilder().build();

    configuration.getLogLevel().ifPresent(HekuLoggingConfigurator::setAllLevels);
    ...
    try {
      Field dbOpAlertThresholdField = LoggingConfigurator.class.getDeclaredField("dbOpAlertThresholdMillis");
      dbOpAlertThresholdField.setAccessible(true);
      dbOpAlertThresholdField.set(null, configuration.getDbOpAlertThresholdMillis());
  ...
  private static void setCustomLevels(final AbstractConfiguration configuration, LoggingConfigExt configEx) {
    for (Pair<String, Level> entry : configEx.getLoggerLevels()) {
      String loggerName = entry.getFirst();
      Level level = entry.getSecond();
  ...
  private static void addFilters(final AbstractConfiguration configuration, LoggingConfigExt configEx) {
    for (Pair<String, Filter> filter : configEx.getFilters()) {
      String loggerName = filter.getFirst();
      LoggerConfig loggerOrParentConfig = configuration.getLoggerConfig(loggerName);
      LoggerConfig loggerConfig;
      if (loggerOrParentConfig.getName().equals(loggerName)) {
        loggerConfig = loggerOrParentConfig;
      } else {
        loggerConfig = new LoggerConfig(loggerName, loggerOrParentConfig.getLevel(), true);
        configuration.addLogger(loggerName, loggerConfig);
      }
      loggerConfig.addFilter(filter.getSecond());
    }
  }
    ...
    switch (destination) {
      ...
      case BOTH:
        ...
        if (consoleLevel != Level.OFF) {
          configuration.getRootLogger().addAppender(consoleAppender, consoleLevel, null);
        }
        break;
    }
  ...
  static LoggingDestination setDestination(final LoggingDestination destination) {
    final LoggingDestination original = HekuLoggingConfigurator.destination;
    HekuLoggingConfigurator.destination = destination;
    return original;
  }
